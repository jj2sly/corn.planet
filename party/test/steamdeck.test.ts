...
describe("Escape Thad's Steam Deck over Socket.IO", () => {
  it("streams runner buttons and Thad's tilt at game speed, and everyone sees the same world", async () => {
    ...
    try {
      ...
      host.on("state", (st) => latest.set(host, st));
      const { code } = await ask(host, "host:create");
      const players: { s: Socket; id: string }[] = [];
      for (const name of ["Thad", "Ann", "Bo"]) {
        const s = await open();
        s.on("state", (st) => latest.set(s, st));
        const joined = await ask(s, "player:join", { code, name });
        players.push({ s, id: joined.playerId });
      }
      ...
      const x = (s: Socket, id: string) => latest.get(s)?.game.world.runners.find((r: View) => r[0] === id)?.[1];
      const [thad, ann] = players as [{ s: Socket; id: string }, { s: Socket; id: string }];
